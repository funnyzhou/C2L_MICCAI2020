...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, low_dim=128, in_channel=3, width=1):
        ...
        self.dropblock = LinearScheduler(
                           DropBlock2D(drop_prob=0.1, block_size=5),
                           start_value=0.,
                           stop_value=0.5,
                           nr_steps=5e3)
        ...
        self.layer4 = self._make_layer(block, self.base * 8, layers[3],
                stride=2, dropblock = self.dropblock)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dropblock=None):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, dropblock=dropblock))
